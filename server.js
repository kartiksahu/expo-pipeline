...
const LinkedInEnhancer = require('./modules/linkedin-enhancer-real');
const EmployeeAnalyzer = require('./modules/employee-analyzer');
// Enhanced analyzers with real fallback mechanisms (preserve originals as backup)
...
async function processCSV(sessionId, filePath) {
    ...
    try {
        ...
        // Debug: Show CSV columns and standardize company names
        if (companies.length > 0) {
            const sampleRow = companies[0];
            console.log('🔍 CSV Columns detected:', Object.keys(sampleRow));
            
            // Standardize company name field across all companies
            ...
            console.log(`✅ Standardized company names. First company: ${companies[0].name}`);
        }
        ...
        try {
            ...
            console.log('Note: Upload file already cleaned up or not found');
        }
...
async function processEmployees(companies, status) {
    ...
    const apiSettings = {
        ...
        rate_limit_ms: 5000  // Increased to 5 seconds to avoid 429 errors
    };
